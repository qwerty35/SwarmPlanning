...
int dimx, dimy, dimz;
std::vector<double> startx, starty, startz, goalx, goaly, goalz;
...
void octomapCallback(const octomap_msgs::Octomap octomap_msg)
{
    ...
    int x,y,z;
    float dist;
    for (double k = z_min; k <= z_max; k += z_res ){
        for (double i = x_min; i <= x_max; i += xy_res ){
            for (double j = y_min; j <= y_max; j += xy_res ){
                octomap::point3d cur_point(i,j,k);
                dist = distmap.getDistance(cur_point);

                if (dist < margin){
                    x = round((i-x_min)/xy_res);
                    y = round((j-y_min)/xy_res);
                    z = round((k-z_min)/z_res);
...
{
    if(_has_init_poses || !_has_octomap)
        return;

    double xi,yi,zi,xf,yf,zf , xig,yig,zig,xfg,yfg,zfg;
    for(int i = 0; i<init_poses_msg.poses.size(); i++){
        xi = init_poses_msg.poses.at(i).position.x;
        yi = init_poses_msg.poses.at(i).position.y;
        zi = init_poses_msg.poses.at(i).position.z;
        xf = init_poses_msg.poses.at(i).orientation.x;
        yf = init_poses_msg.poses.at(i).orientation.y;
        zf = init_poses_msg.poses.at(i).orientation.z;

        startx.emplace_back(xi);
        starty.emplace_back(yi);
        startz.emplace_back(zi);

        goalx.emplace_back(xf);
        goaly.emplace_back(yf);
        goalz.emplace_back(zf);

        xig = round((xi-x_min)/xy_res);
        yig = round((yi-y_min)/xy_res);
        zig = round((zi-z_min)/z_res);
        xfg = round((xf-x_min)/xy_res);
        yfg = round((yf-y_min)/xy_res);
        zfg = round((zf-z_min)/z_res);
        ...
        goals.emplace_back(Location(xfg,yfg,zfg));
    }
...
int main(int argc, char* argv[]) {
    ...
    n.param<double>("world/z_size", z_size, 4);

    n.param<double>("ecbs/w", w, 1.3);
    n.param<double>("ecbs/xy_res", xy_res, 1);
    n.param<double>("ecbs/z_res", z_res, 1.5);
    n.param<float>("ecbs/margin", margin, 2.99);
    ...
    dimx = (x_max - x_min) / xy_res + 1;
    dimy = (y_max - y_min) / xy_res + 1;
    dimz = (z_max - z_min) / z_res + 1;
            ...
            if (success) {
                ...
                for (size_t a = 0; a < solution.size(); ++a) {
                    ...
                    for (const auto &state : solution[a].states) {
                        sol.pose.position.x = state.first.x * xy_res + x_min;
                        sol.pose.position.y = state.first.y * xy_res + y_min;
                        sol.pose.position.z = state.first.z * z_res + z_min;
                        _ecbs_path.poses.push_back(sol);
                    }
