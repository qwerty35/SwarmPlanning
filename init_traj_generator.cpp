...
void initPosesCallback(const geometry_msgs::PoseArray init_poses_msg)
{
    ...
    for(int i = 0; i<init_poses_msg.poses.size(); i++){
        ...
        startStates.emplace_back(State(0, round((startx.at(i)-x_min)/xy_res), round((starty.at(i)-y_min)/xy_res), round((startz.at(i)-z_min)/z_res)));
        goals.emplace_back(Location(round((goalx.at(i)-x_min)/xy_res), round((goaly.at(i)-y_min)/xy_res), round((goalz.at(i)-z_min)/z_res)));
    }
...
int main(int argc, char* argv[]) {
    ...
    n.param<double>("ecbs/xy_res", xy_res, 0.6);
    n.param<double>("ecbs/z_res", z_res, 1.8);
    n.param<float>("ecbs/margin", margin, 0.3);
